...
const renderStatus = card => {
  ...
  if (type === 'courier') { selectedStatus = type; transferDetail.innerHTML = details.courier; transferDetail.className = 'status-detail show transfer-standalone'; transferDetail.querySelector('[data-ticket-transfer]')?.addEventListener('click', () => { window.location.href = 'ticket/?category=probleme_admin&type=transfer_cont'; }); show('transfer-flow'); return; }
