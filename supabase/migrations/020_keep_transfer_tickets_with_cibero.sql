-- Account-transfer requests are handled centrally by the CibeRO admin, even
-- when the applicant already belongs to a sub-fleet.
...
comment on function public.route_ticket_to_subfleet() is
  'Routes courier tickets to the owning sub-fleet, except account transfers which stay with CibeRO.';
