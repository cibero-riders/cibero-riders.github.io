...
if (requestedPlatform === 'glovo') show('glovo-info-flow');
if (requestedPlatform === 'wolt') show('wolt-flow');
...
const renderStatus = card => {
  ...
  if (type === 'courier') {
    selectedStatus = type;
    transferDetail.innerHTML = details.courier;
    transferDetail.className = 'status-detail show transfer-standalone';
    const transferCheck = transferDetail.querySelector('.transfer-check');
    const transferButton = transferDetail.querySelector('[data-ticket-transfer]');
    transferCheck?.addEventListener('change', () => { transferButton.disabled = !transferCheck.checked; });
    transferButton?.addEventListener('click', () => { window.location.href = 'ticket/?category=probleme_admin&type=transfer_cont&criteria=confirmed&platform=wolt'; });
    show('transfer-flow');
    return;
  }
